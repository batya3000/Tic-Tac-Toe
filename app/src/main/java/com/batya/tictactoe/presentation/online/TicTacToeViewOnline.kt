...
import com.batya.tictactoe.domain.model.OnFieldChangedListener
...
class TicTacToeViewOnline(
    ...
    var lastTurn: Pair<Int, Int> = Pair(-1, -1)
        set(value) {
            field = value
            drawLastCell = true
            invalidate()
    ...
    private fun drawLastCell(canvas: Canvas) {
        if (lastTurn.first != 1 || lastTurn.second != -1) {
            canvas.drawRect(getLastCellRect(lastTurn.first, lastTurn.second), lastCellPaint)
        }
    }
